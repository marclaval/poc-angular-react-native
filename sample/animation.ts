...
var ballsRegistry : Array<Ball> = [];

@Component({
  selector: 'ball',
  host: {position: 'absolute', top: '0', left: '0', bottom: '0', right: '0'},
  template: `
<View [style]="styles.ball" [top]="_y" [left]="_x" [backgroundColor]="_color" [borderRadius]="_radius" [width]="_radius*2" [height]="_radius*2" (pan)="moveBall($event)" (panend)="endMoveBall($event)"></View>
`
})
export class Ball {
...
@Component({
  ...
  template: `
<ball *ngFor="#ball of balls" x="{{ball.x}}" y="{{ball.y}}" color="{{ball.color}}" radius="{{ball.radius}}" (tap)="moveAll()"></ball>
`
})
export class AnimationApp {
  ...
  moveAll() {
    ballsRegistry.forEach((ball: Ball) => {
