...
export class ReactNativeRootRenderer extends RootRenderer {
...
export class ReactNativeRenderer extends Renderer {
  ...
  private _propsEater: Map<Node, NativeCommandCreate | NativeCommandUpdate> = new Map();
  ...
  createElement(parentElement: Node, name: string): Node {
    var node = new ElementNode(name, false, {}, this._rootRenderer.wrapper, this._rootRenderer.zone);
    if (parentElement) {
  ...
  attachViewAfter(node: Node, viewRootNodes: Node[]): void {
    if (viewRootNodes.length > 0) {
      ...
      for (var i = 0; i < viewRootNodes.length; i++) {
        ...
        node.parent.children.splice(index + i + 1, 0, viewRootNode);
        viewRootNode.parent = node.parent;
  ...
  setText(renderNode: Node, text: string): void {
    if (renderNode instanceof TextNode) {
      ...
      renderNode.setText(text);
      this.setElementProperty(renderNode, 'text', text);
    }
  }
