...
export class TextInput extends HighLevelComponent implements OnInit {
  ...
  //Event handlers
  _handleFocus() { this.focus.emit(null); }
  _handleChange(event: any) { this.change.emit(event.text); }
  _handleTextInput(event: any) { this.input.emit(event.text); }
  _handleSubmitEditing(event: any) { this.submit.emit(event.text); }
  _handleBlur() { this.blur.emit(null); }
  _handleEndEditing(event: any) { this.endEditing.emit(event.text); }
