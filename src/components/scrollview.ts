...
@Component({
  ...
  template: `
  <native-scrollview ${isAndroid() ? '*ngIf="!_horizontal"' : ''} [horizontal]="_horizontal" [keyboardDismissMode]="_keyboardDismissMode" [keyboardShouldPersistTaps]="_keyboardShouldPersistTaps"
  ...
  (topScroll)="_handleScroll($event)"
  ${GENERIC_BINDINGS} ${isAndroid() ? ANDROID_BINDINGS : IOS_BINDINGS}><native-view [removeClippedSubviews]="_removeClippedSubviews" [style]="_contentContainerStyle" collapsable="false">
  <ng-content></ng-content></native-view></native-scrollview>
  ${isAndroid() ? `<native-horizontalscrollview *ngIf="_horizontal" [horizontal]="_horizontal" [keyboardDismissMode]="_keyboardDismissMode" [keyboardShouldPersistTaps]="_keyboardShouldPersistTaps"
  [scrollEnabled]="_scrollEnabled" [showsHorizontalScrollIndicator]="_showsHorizontalScrollIndicator" [showsVerticalScrollIndicator]="_showsVerticalScrollIndicator"
  (topScroll)="_handleScroll($event)"
  ${GENERIC_BINDINGS} ${isAndroid() ? ANDROID_BINDINGS : IOS_BINDINGS}><native-view [removeClippedSubviews]="_removeClippedSubviews" [style]="_contentContainerStyle" collapsable="false">
  <ng-content></ng-content></native-view></native-horizontalscrollview>`: ``}`
})
export class ScrollView extends HighLevelComponent{
  ...
  //Commands
  scrollTo(x: number) {
    this._nativeElement.children[0].dispatchCommand('scrollTo', [x]);
  }

  scrollWithoutAnimationTo(x: number) {
    this._nativeElement.children[0].dispatchCommand('scrollWithoutAnimationTo', [x]);
  }
}
