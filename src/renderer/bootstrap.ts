...
import {DrawerLayout, DrawerLayoutNavigation, DrawerLayoutContent} from "../components/android/drawer_layout";

export function bootstrapReactNative(appName:string, cpt: any) {
  ReactNativeWrapperImpl.registerApp(appName, function() {
    enableProdMode();
    bootstrap(cpt, [
      provide(PLATFORM_DIRECTIVES, {useValue: [View, Text, Switch, TextInput, WebView, ProgressBar, PagerLayout, DrawerLayout, DrawerLayoutNavigation, DrawerLayoutContent], multi:true}),
      ROUTER_PROVIDERS,
